      ...
      <main className="flex-grow flex items-center justify-center px-4 pt-32 pb-20 w-full">
        <div className="w-full max-w-md bg-[#0f0f0f] border border-slate-800 rounded-3xl p-8 shadow-2xl relative z-10">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">We'll send you a link to reset it</p>
          </div>
          ...
          {message && <div className="bg-green-500/10 border border-green-500/50 text-green-400 p-3 rounded-xl mb-6 text-sm text-center">{message}</div>}
          ...
          <form onSubmit={handleReset} className="flex flex-col gap-5">
            ...
            <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 rounded-xl transition-all mt-2 disabled:opacity-50">
              {loading ? "Sending..." : "Send Reset Link"}
            </button>
          </form>

          <div className="mt-8 text-center text-sm text-slate-400">
            Remembered your password? <Link href="/sign-in" className="text-white font-bold hover:text-blue-400 transition-colors">Sign in</Link>
          </div>
        </div>
      </main>
    </div>
